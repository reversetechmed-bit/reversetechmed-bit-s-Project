...
export async function exportHandoverInvoicePdf(record: InvoiceRecord) {
  ...
  pdf.setFontSize(11); pdf.setTextColor(90, 87, 78); pdf.text(shapePdfText(pdf, `رقم الفاتورة: ${invoice.invoiceNumber}`), right, 162, { align: "right" }); pdf.text(shapePdfText(pdf, `التاريخ: ${issued.date}`), right, 181, { align: "right" }); pdf.text(shapePdfText(pdf, `الوقت: ${issued.time}`), right, 200, { align: "right" });
  ...
  autoTable(pdf, { startY: 220, body: partyRows.map(row => row.map(value => shapePdfText(pdf, value))), theme: "grid", styles: { font: "NotoNaskh", fontSize: 10, cellPadding: 7, halign: "right", textColor: [23, 55, 76], lineColor: [217, 208, 191], lineWidth: 0.45 }, columnStyles: { 0: { cellWidth: 130, fillColor: [244, 240, 232], fontStyle: "bold" }, 1: { cellWidth: 380 } }, margin: { left: 42, right: 42 } });
  const tableY = (pdf as any).lastAutoTable.finalY + 18;
  autoTable(pdf, { startY: tableY, head: [["كود الصنف", "اسم الصنف", "قسم المخزن", "الكمية"]].map(row => row.map(value => shapePdfText(pdf, value))), body: [[invoice.partNumberSnapshot, invoice.partNameSnapshot, sectionLabel(invoice.warehouseSectionSnapshot), `${invoice.quantity} وحدة`]].map(row => row.map(value => shapePdfText(pdf, value))), theme: "grid", styles: { font: "NotoNaskh", fontSize: 10, cellPadding: 7, halign: "right", textColor: [23, 55, 76], lineColor: [217, 208, 191], lineWidth: 0.45 }, headStyles: { fillColor: [23, 55, 76], textColor: [255, 255, 255] }, margin: { left: 42, right: 42 } });
  ...
  autoTable(pdf, { startY: detailsY, body: details.map(row => row.map(value => shapePdfText(pdf, value))), theme: "grid", styles: { font: "NotoNaskh", fontSize: 9.5, cellPadding: 6, halign: "right", textColor: [23, 55, 76], lineColor: [217, 208, 191], lineWidth: 0.45 }, columnStyles: { 0: { cellWidth: 130, fillColor: [244, 240, 232], fontStyle: "bold" }, 1: { cellWidth: 380 } }, margin: { left: 42, right: 42 } });
  ...
  pdf.setTextColor(107, 106, 96); pdf.setFontSize(8.5); pdf.text(shapePdfText(pdf, "REVERSE TECH · وثيقة تشغيلية داخلية"), pageWidth / 2, pageHeight - 26, { align: "center" });
