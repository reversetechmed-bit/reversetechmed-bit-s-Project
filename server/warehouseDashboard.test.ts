...
function chain(result: unknown[]) {
  ...
  return { from: vi.fn(() => ({ orderBy: orderedQuery, where: vi.fn(() => ({ orderBy: orderedQuery })), leftJoin: vi.fn(() => ({ orderBy: orderedQuery })), innerJoin: vi.fn(() => ({ orderBy: orderedQuery })) })) };
}
...
describe("warehouse dashboard output", () => {
  it("returns recent warehouse activity and most recent user access alongside stock summaries", async () => {
    ...
    expect(dashboard).toMatchObject({ partCount: 2, totalUnits: 10, reservedUnits: 0, availableUnits: 10, componentCount: 1, productCount: 1, pendingRequests: 1, lowStockParts: [{ id: 1 }], unreadAlerts: [{ id: 5 }], topDispensedParts: [], recentHandovers: [] });
